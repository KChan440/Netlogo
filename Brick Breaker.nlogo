...
breed [balls ball]
breed [blocks block]
globals [score lives is-running?]
patches-own [nextcolor]

to setup
  ca
  create-balls 1 [ball-setup]
  paddle-setup
  block-setup
  set lives 5
  set score 0
...
to lose-screen
  if lives = 0 [ask turtles [set shape "face sad"] ask patch 0 13 [set plabel "YOU LOSE!"] 
    ask patch 0 13 [set plabel-color white]]
end
...
to ballrules
if [pcolor] of patch-ahead 1 = green [set heading (heading + 180)]
if pycor < -13 [set is-running? false]
end
to ball-setup
  ask balls
  [set shape "circle"
...
to boundaries
  ask patches with [abs pxcor = 16] [set pcolor green]
  ask patches with [abs pycor = 16][set pcolor green]
end

to paddle-setup
  ask patches with [pxcor = -2 and pycor = -13][set pcolor green]
  ask patches with [pxcor = -1 and pycor = -13][set pcolor white]
  ask patches with [pxcor = 0 and pycor = -13][set pcolor blue]
  ask patches with [pxcor = 1 and pycor = -13][set pcolor red]
  ask patches with [pxcor = 2 and pycor = -13][set pcolor yellow]
  

end

to block-setup
  ask patches with [ abs pycor < 3 and abs pxcor < 15 ] [ sprout-blocks 1]
ask blocks [set color red
...
true
0
Rectangle -7500403 true true 45 150 255 195
